...
resource "aws_cloudfront_distribution" "website" {
  ...
  default_cache_behavior {
    ...
    # Using managed cache policy for better performance
    cache_policy_id          = "658327ea-f89d-4fab-a63d-7e88639e58f6" # CachingOptimized
    origin_request_policy_id = "88a5eaf4-2fd4-4709-b370-b4c650ea3fcf" # CORS-S3Origin

    # Function associations for URL rewriting
...
# CloudFront function for URL rewriting (handles trailing slashes)
resource "aws_cloudfront_function" "url_rewrite" {
  name    = "${var.project_name}-url-rewrite"
  runtime = "cloudfront-js-2.0"
  comment = "URL rewriting for static site"
  publish = true
  code    = <<-EOF
    function handler(event) {
      ...
      // Check if URI is missing a file name
      if (uri.endsWith('/')) {
